...
!
  ...
  subroutine write_partition_files

! reads in proc***_Databases files
  use constants, only: &
          NDIM, NGNOD, NGNOD2D, NGLLX, NGLLY, NGLLZ, IMAIN, IOUT
  ...
  integer :: num_moho
  integer :: i,j,ie,ier
  integer :: dummy_node, imat, ispec, inode, dummy_elmnt, ispec2D

! read databases about external mesh simulation
! global node coordinates
  call create_name_database_cartesian(prname,myrank,LOCAL_PATH)
  open(unit=IOUT,file=prname(1:len_trim(prname))//'Database', &
...
! added poroelastic properties and filled with 0 the last 10 entries for elastic/acoustic
  write(IOUT) nmat_ext_mesh, nundefMat_ext_mesh
  ...
  do imat = 1, nmat_ext_mesh
     ...
     write(IOUT) materials_ext_mesh(1,imat),  materials_ext_mesh(2,imat),  materials_ext_mesh(3,imat), &
          ...
          materials_ext_mesh(16,imat), materials_ext_mesh(17,imat)
  enddo
  ...
  do imat = 1, nundefMat_ext_mesh
     ...
     write(IOUT) undef_mat_prop(1,imat),undef_mat_prop(2,imat),undef_mat_prop(3,imat),undef_mat_prop(4,imat), &
          undef_mat_prop(5,imat), undef_mat_prop(6,imat)
  enddo
  deallocate(undef_mat_prop)
...
! element indexing
  ...
  do ispec = 1, nelmnts_ext_mesh
     ...
     write(IOUT) ispec, mat_ext_mesh(1,ispec),mat_ext_mesh(2,ispec),(elmnts_ext_mesh(j,ispec),j=1,NGNOD)

     ! check debug
     !if (dummy_elmnt /= ispec) stop 'Error ispec order in materials file'

  enddo
  deallocate(elmnts_ext_mesh, mat_ext_mesh)
...
! reads absorbing/free-surface boundaries
  ...
  do ispec2D = 1,nspec2D_xmin
     write(IOUT) ibelm_xmin(ispec2D),(nodes_ibelm_xmin(j,ispec2D),j=1,NGNOD2D)
  enddo
  ...
  do ispec2D = 1,nspec2D_xmax
     write(IOUT) ibelm_xmax(ispec2D),(nodes_ibelm_xmax(j,ispec2D),j=1,NGNOD2D)
  enddo
  ...
  do ispec2D = 1,nspec2D_ymin
     write(IOUT) ibelm_ymin(ispec2D),(nodes_ibelm_ymin(j,ispec2D),j=1,NGNOD2D)
  enddo
  ...
  do ispec2D = 1,nspec2D_ymax
     write(IOUT) ibelm_ymax(ispec2D),(nodes_ibelm_ymax(j,ispec2D),j=1,NGNOD2D)
  enddo
  deallocate(ibelm_ymax, nodes_ibelm_ymax)
  ...
  do ispec2D = 1,nspec2D_bottom_ext
     write(IOUT) ibelm_bottom(ispec2D),(nodes_ibelm_bottom(j,ispec2D),j=1,NGNOD2D)
  enddo
  deallocate(ibelm_bottom, nodes_ibelm_bottom)
  ...
  do ispec2D = 1,nspec2D_top_ext
     write(IOUT) ibelm_top(ispec2D),(nodes_ibelm_top(j,ispec2D),j=1,NGNOD2D)
  enddo
  deallocate(ibelm_top, nodes_ibelm_top)
  ...
  if (nspec_cpml_tot > 0) then
     ...
     do i=1,nspec_cpml
        ...
        write(IOUT) CPML_to_spec(i), CPML_regions(i)
     enddo
     deallocate(CPML_to_spec, CPML_regions)
...
! MPI interfaces between different partitions
  ...
  !  write(IOUT) num_interfaces_ext_mesh, max_interface_size_ext_mesh
  !else
  ...
  do num_interface = 1, num_interfaces_ext_mesh
    ...
    write(IOUT) my_neighbors_ext_mesh(num_interface), my_nelmnts_neighbors_ext_mesh(num_interface)
    ...
    do ie = 1, my_nelmnts_neighbors_ext_mesh(num_interface)
      ...
      !     2  -  element edge
      !     4  -  element face
      write(IOUT) my_interfaces_ext_mesh(1,ie,num_interface), my_interfaces_ext_mesh(2,ie,num_interface), &
    ...
    enddo
  enddo
  deallocate(my_neighbors_ext_mesh, my_nelmnts_neighbors_ext_mesh, &
          my_interfaces_ext_mesh, ibool_interfaces_ext_mesh, &
  ...
  if (SAVE_MOHO_MESH) then
    ...
    write(IOUT,iostat=ier) boundary_number,nspec2D_moho_ext
    if (ier /= 0) then
    ...
    do ispec2D = 1,nspec2D_moho_ext
      ...
      write(IOUT) ibelm_moho(ispec2D),(nodes_ibelm_moho(j,ispec2D),j=1,NGNOD2D)
    enddo
    deallocate(ibelm_moho, nodes_ibelm_moho)
  ...
  end subroutine write_partition_files
