...
! reads in proc***_Databases files
  use constants, only: &
          NDIM, NGNOD, NGNOD2D, NGLLX, NGLLY, NGLLZ, IMAIN, IIN
...
! global node coordinates
  ...
  do inode = 1, npts
     read(IIN) dummy_node, nodes_coords(1,inode), nodes_coords(2,inode), &
...
! added poroelastic properties and filled with 0 the last 10 entries for elastic/acoustic
  ...
  allocate(materials_ext_mesh(17,nmat_ext_mesh),stat=ier)
