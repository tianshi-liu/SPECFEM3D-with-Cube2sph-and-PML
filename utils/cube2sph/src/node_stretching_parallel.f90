program node_stretching_parallel
   use mpi
   use constants
   ...
   !write(IOUT, *) npts
   !do ipts = 1, npts
   !  write(IOUT, *) ipts, xnode_new(ipts), ynode_new(ipts), znode_new(ipts)
   !enddo
   !close(IOUT)
   !!!!! output !!!!!!!
   ...
   !!!!!!!!!!!!!!!!!!!!
   deallocate(nodes_coords_new, idoubling, ispec_is_tiso)
   call MPI_Finalize(ier)
end program node_stretching_parallel
