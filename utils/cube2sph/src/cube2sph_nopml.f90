...
program cube2sph
  ...
  !  call calc_adepml_physical_jacobian(num_pml_physical,xstore_sph,ystore_sph,&
  !        zstore_sph,NSPEC_AB,pml_physical_ispec,pml_physical_ijk,&
  ...
  !  allocate(rvolume_loc(NGLLX,NGLLY,NGLLZ,nspec_cpml))
    ...
    !r_trans_inv(2,2,:,:,:,:) = 1.0
    !r_trans_inv(3,3,:,:,:,:) = 1.0
    !call calc_r_trans(nodes_coords,nodes_coords_sph,npts,nspec_cpml,&
    !                  r_trans,r_trans_inv)
  ...
  deallocate(xstore,ystore,zstore,xstore_sph,ystore_sph,zstore_sph,jacobian3D)
  !if (nspec_cpml > 0) &
