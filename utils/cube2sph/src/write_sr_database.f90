program write_stations_file
  ...
  do irec = 1,nrec
    ...
    read(string(1:len_trim(string)),*,iostat=ier) network_name(irec),station_name(irec), &
  ...
  do irec = 1,nrec
    ...
    write(IOUT, "(a7,a4,f23.4,f23.4,f23.4,f23.9,f23.9,f23.9)") &
            station_name(irec), network_name(irec), &
            x_target(irec), y_target(irec), z_target(irec), &
            nu(3,1,irec),nu(3,2,irec),nu(3,3,irec)
  enddo
  close(IOUT)
