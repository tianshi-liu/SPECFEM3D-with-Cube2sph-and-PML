...
! returns: xlon,xlat,weight
  ...
  subroutine smooth_weights_CAP_vardegree_gaussian(lon,lat,xlon,xlat,weight,CAP_DEGREE,NTHETA,NPHI)
...
! returns: xlon,xlat,weight
  ...
  CAP = CAP_DEGREE * DEGREES_TO_RADIANS
  CAP3 = CAP * 3.0
  dtheta = 0.5d0 * CAP3 / dble(NTHETA)
  dphi = TWO_PI / dble(NPHI)

  !cap_area = TWO_PI * ( ONE - dcos(CAP) )
  !dweight = CAP / dble(NTHETA) * dphi / cap_area
  dweight = CAP3 / dble(NTHETA) * dphi
  pi_over_nphi = PI/dble(NPHI)
  ...
  do itheta = 1,NTHETA
    ...
    cost = dcos(theta)
    sint = dsin(theta)
    wght = sint*dweight*exp(-theta*theta/(CAP*CAP))
  ...
  end subroutine smooth_weights_CAP_vardegree_gaussian
