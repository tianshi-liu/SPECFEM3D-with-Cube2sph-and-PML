...
!
  ...
  subroutine get_topo_bathy_point(xlat,xlon,value,ibathy_topo)
...
! Use bilinear interpolation rather nearest point interpolation
  ...
  end subroutine get_topo_bathy_point


!!!!!!TL: make a wrapper of subroutine get_topo_bathy
!!!!!! the original content of subroutine becomes get_topo_bathy_point
  subroutine get_topo_bathy(lat,lon,value,ibathy_topo)
...
!
  ...
  ! local parameter
  logical :: cap_smooth
  double precision, parameter :: cap_degree = 0.2
  integer, parameter :: NTHETA = 20, NPHI = 60
  double precision :: xlon(NTHETA*NPHI),xlat(NTHETA*NPHI),weight(NTHETA*NPHI)
  integer :: i
  double precision :: weightl, valuel
  !!!!!!!!!!!!!!!!!!!!!!!!
  cap_smooth = .true.
  if (cap_smooth) then
    value = ZERO
    call smooth_weights_CAP_vardegree_gaussian(lon,lat,xlon,xlat,weight,&
            cap_degree,NTHETA,NPHI)
    do i = 1, NTHETA*NPHI
      call get_topo_bathy_point(xlat(i),xlon(i),valuel,ibathy_topo)
      weightl = weight(i)
      value = value + weightl * valuel
    enddo
  else
    call get_topo_bathy_point(lat,lon,value,ibathy_topo)
  endif
