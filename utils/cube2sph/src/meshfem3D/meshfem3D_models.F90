...
!! TL: add subroutine for deallocation
  subroutine meshfem3D_crust_deallocate()
  use meshfem3D_models_par
  implicit none
  select case (ITYPE_CRUSTAL_MODEL)
    case (ICRUST_CRUST1)
      ! crust 1.0
      call model_crust_1_0_deallocate()
    case default
      stop 'crustal model other than crust1.0 not supported for deallocation'
  end select
  end subroutine meshfem3D_crust_deallocate
...
!---
  ...
  select case (ITYPE_CRUSTAL_MODEL)
    ...
    case (ICRUST_CRUST1)
      ! crust 1.0
      call model_crust_1_0_smooth(lat,lon,r,vpc,vsc,rhoc,moho,found_crust,elem_in_crust)
      vpvc = vpc
      vphc = vpc
      vsvc = vsc
      vshc = vsc
