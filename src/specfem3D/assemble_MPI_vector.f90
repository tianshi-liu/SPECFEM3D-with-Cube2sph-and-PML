...
!
  ...
  !!TL: add this modified subroutine for ADE-PML

  subroutine assemble_MPI_matrix_async_send(NPROC,NGLOB_AB,array_val, &
  ...
  ! array to assemble
  real(kind=CUSTOM_REAL), dimension(NDIM,NDIM,NGLOB_AB) :: array_val
  ...
  integer, dimension(num_interfaces) :: request_send_matrix,request_recv_matrix
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, num_interfaces
      do ipoin = 1, nibool_interfaces(iinterface)
        buffer_send_matrix(:,:,ipoin,iinterface) = &
...
! wait for communications completion (send)
  ...
  !! TL: add this modified subroutine for ADE-PML
  subroutine assemble_MPI_matrix_async_w_ord(NPROC,NGLOB_AB,array_val, &
                        buffer_recv_matrix,num_interfaces, &
                        max_nibool_interfaces, &
                        nibool_interfaces,ibool_interfaces, &
                        request_send_matrix,request_recv_matrix, &
                        my_neighbors,myrank,t_wait)
  ! TL: timing
! waits for data to receive and assembles
...
! array to assemble
  ...
  integer :: num_interfaces,max_nibool_interfaces,myrank
  ...
  real(kind=CUSTOM_REAL), dimension(NDIM,NDIM,max_nibool_interfaces,num_interfaces) :: mybuffer
  integer :: ipoin,iinterface,iglob
  logical :: need_add_my_contrib
  ! TL: timing
  double precision :: t_wait, t_clock
...
! wait for communications completion (recv)
  do iinterface = 1, num_interfaces
    call wait_req(request_recv_matrix(iinterface))
  enddo
...
! adding all contributions in order of processor rank
  ...
  do iinterface = 1, num_interfaces
    ...
    do ipoin = 1, nibool_interfaces(iinterface)
      iglob = ibool_interfaces(ipoin,iinterface)
      array_val(:,:,iglob) = array_val(:,:,iglob) + buffer_recv_matrix(:,:,ipoin,iinterface)
    enddo
  enddo
  if (need_add_my_contrib) call add_my_contrib()
...
! wait for communications completion (send)
  do iinterface = 1, num_interfaces
    call wait_req(request_send_matrix(iinterface))
  enddo
  ! TL: timing
  t_wait = t_wait + (wtime() - t_clock)
  ...
  end subroutine assemble_MPI_matrix_async_w_ord
