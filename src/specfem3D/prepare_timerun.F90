...
!
  ...
  if (ELASTIC_SIMULATION) then
    ...
    if (PML_CONDITIONS .and. USE_ADE_PML .and. (nglob_CPML>0)) then
      !do iglob = 1, nglob_CPML
      !  rvolume_ext(CPML_to_glob(iglob)) = rvolume(iglob)
      !enddo
      call assemble_MPI_scalar_blocking(NPROC,nglob_CPML,rvolume, &
              num_interfaces_PML,max_nibool_interfaces_PML, &
              nibool_interfaces_PML,ibool_interfaces_PML, &
              my_neighbors_PML)
      !call wait_for_attach(myrank)
      !do iglob = 1, nglob_CPML
      !  rvolume(iglob) = rvolume_ext(CPML_to_glob(iglob))
      !enddo
      where (rvolume <= 0._CUSTOM_REAL) rvolume = 1._CUSTOM_REAL
      rvolume(:) = 1._CUSTOM_REAL / rvolume(:)
    endif
...
!   stop 'error: the C-PML code works for 8-node bricks only; should be made more general'
  ...
  !! TL: to accomodate ADE-PML
  if (.not. USE_ADE_PML) then
    call pml_allocate_arrays()
  else
    call adepml_allocate_arrays()
    call prepare_adepml_update_coeff()
  endif
...
! using snapshot files of wavefields
  ...
  !if (STACEY_ABSORBING_CONDITIONS) then
  if (STACEY_ABSORBING_CONDITIONS .and. &
