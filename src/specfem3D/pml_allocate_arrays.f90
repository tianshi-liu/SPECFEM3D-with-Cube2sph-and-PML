...
! acoustic domains
  ...
  !! TL: ADE-PML
  subroutine adepml_allocate_arrays()
  use pml_par
  ...
  if (NSPEC_CPML == 0) then
    ! dummy allocation with a size of 1 for all the PML arrays that have not yet been allocated
    ! in order to be able to use these arrays as arguments in subroutine calls
    call adepml_allocate_arrays_dummy()
    return
  endif
  ...
  ! ADE-PML auxiliary variables
  allocate(Qu(NDIM,NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu')
  if (ier /= 0) stop 'error allocating array Qu'
  allocate(Qu_t(NDIM,NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu_t')
  if (ier /= 0) stop 'error allocating array Qu_t'
  !allocate(pml_sigma(6,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array pml_sigma')
  !if (ier /= 0) stop 'error allocating array Qu_t'
  !allocate(Qu_t0(NDIM,NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu_t0')
  !if (ier /= 0) stop 'error allocating array Qu_t0'
  allocate(Qt(NDIM,NDIM,nglob_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt')
  if (ier /= 0) stop 'error allocating array Qt'
  allocate(Qt_t(NDIM,NDIM,nglob_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt_t')
  if (ier /= 0) stop 'error allocating array Qt_t'
  !allocate(Qt_t0(NDIM,NDIM,nglob_CPML),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt_t0')
  !if (ier /= 0) stop 'error allocating array Qt_t0'

  ! convolution update coefficients
  allocate(coeff_exp1(NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp1')
  if (ier /= 0) stop 'error allocating array coeff_exp1'
  allocate(coeff_exp2(NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp2')
  if (ier /= 0) stop 'error allocating array coeff_exp2'
  !allocate(coeff_exp3(NDIM,NGLLX,NGLLY,NGLLZ,NSPEC_CPML),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp3')
  !if (ier /= 0) stop 'error allocating array coeff_exp3'

  allocate(coeff_glob_exp1(NDIM,nglob_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp1')
  if (ier /= 0) stop 'error allocating array coeff_glob_exp1'
  allocate(coeff_glob_exp2(NDIM,nglob_CPML),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp2')
  if (ier /= 0) stop 'error allocating array coeff_glob_exp2'
  !allocate(coeff_glob_exp3(NDIM,nglob_CPML),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp3')
  !if (ier /= 0) stop 'error allocating array coeff_glob_exp3'
  ...
  Qu(:,:,:,:,:,:) = 0._CUSTOM_REAL
  Qu_t(:,:,:,:,:,:) = 0._CUSTOM_REAL
  !Qu_t0(:,:,:,:,:,:) = 0._CUSTOM_REAL
  Qt(:,:,:) = 0._CUSTOM_REAL
  Qt_t(:,:,:) = 0._CUSTOM_REAL
  !Qt_t0(:,:,:) = 0._CUSTOM_REAL
  !pml_sigma(:,:,:,:,:) = 0._CUSTOM_REAL
...
!=====================================================================
  ...
  subroutine adepml_allocate_arrays_dummy()
  use pml_par
  ...
  if (.not. allocated(Qu)) then
    allocate(Qu(1,1,1,1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu')
  endif
  if (.not. allocated(Qu_t)) then
    allocate(Qu_t(1,1,1,1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu_t')
  endif
  !if (.not. allocated(pml_sigma)) then
  !  allocate(pml_sigma(1,1,1,1,1),stat=ier)
  !  if (ier /= 0) call exit_MPI_without_rank('error allocating array pml_sigma')
  !endif
  !if (.not. allocated(Qu_t0)) then
  !  allocate(Qu_t0(1,1,1,1,1,1),stat=ier)
  !  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qu_t0')
  !endif
  if (.not. allocated(Qt)) then
    allocate(Qt(1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt')
  endif
  if (.not. allocated(Qt_t)) then
    allocate(Qt_t(1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt_t')
  endif
  !if (.not. allocated(Qt_t0)) then
  !  allocate(Qt_t0(1,1,1),stat=ier)
  !  if (ier /= 0) call exit_MPI_without_rank('error allocating array Qt_t0')
  !endif
  if (.not. allocated(coeff_exp1)) then
    allocate(coeff_exp1(1,1,1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp1')
  endif
  if (.not. allocated(coeff_exp2)) then
    allocate(coeff_exp2(1,1,1,1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp2')
  endif
  !if (.not. allocated(coeff_exp3)) then
  !  allocate(coeff_exp3(1,1,1,1,1),stat=ier)
  !  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_exp3')
  !endif
  if (.not. allocated(coeff_glob_exp1)) then
    allocate(coeff_glob_exp1(1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp1')
  endif
  if (.not. allocated(coeff_glob_exp2)) then
    allocate(coeff_glob_exp2(1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp2')
  endif
  !if (.not. allocated(coeff_glob_exp3)) then
  !  allocate(coeff_glob_exp3(1,1),stat=ier)
  !  if (ier /= 0) call exit_MPI_without_rank('error allocating array coeff_glob_exp3')
  !endif
  ...
  end subroutine adepml_allocate_arrays_dummy
...
! deallocates C_PML arrays
  ...
  if (NSPEC_CPML > 0) then
    ...
    if (.not. USE_ADE_PML) then
      ...
      deallocate(pml_d,pml_kappa,pml_beta)
    endif
  endif
  ...
  if (USE_ADE_PML) then
    deallocate(Qu,Qu_t,Qt,Qt_t)
    deallocate(coeff_exp1,coeff_exp2)
    deallocate(coeff_glob_exp1,coeff_glob_exp2)
    deallocate(accel_elastic_CPML)
  endif
  if (ELASTIC_SIMULATION .and. (.not. USE_ADE_PML)) then
