...
! elastic term
    ...
    if (iphase == 1) then
      ...
      !! TL: calculate the contributions of the intersections of PML and !!
      !! physical boundaries, to update PML auxiliary variables,         !!
      !! note that all such contributions are included in phase 1 to     !!
      !! avoid redundancy                                                !!
      !if(PML_CONDITIONS .and. USE_ADE_PML) call compute_add_pml_physical()
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (iphase == 1) then
       ...
       !! TL: exchange data between processors for !!
       !! ADE-PML auxiliary variable !!
       if (PML_CONDITIONS .and. USE_ADE_PML.and.(nglob_CPML>0)) then
         !do igll = 1, nglob_CPML
         !  Qt_t_ext(:,:,CPML_to_glob(igll)) = Qt_t(:,:,igll)
         !enddo
         call assemble_MPI_matrix_async_send(NPROC,nglob_CPML,Qt_t, &
               buffer_send_matrix_PML,buffer_recv_matrix_PML, &
               num_interfaces_PML,max_nibool_interfaces_PML, &
               nibool_interfaces_PML,ibool_interfaces_PML, &
               my_neighbors_PML, &
      ...
      !if (myrank==0) then
      !   print *, 'nglob_CPML=',nglob_CPML
      !   print *, 'num_interfaces_PML=',num_interfaces_PML
      !   print *, nibool_interfaces_PML
      !endif
      ...
      call assemble_MPI_vector_async_w_ord(NPROC,NGLOB_AB,accel, &
               ...
               request_send_vector_ext_mesh,request_recv_vector_ext_mesh, &
               my_neighbors_ext_mesh,myrank)
      !! TL: wait and assemble for ADE-PML auxiliary variable !!
      if (PML_CONDITIONS .and. USE_ADE_PML.and.(nglob_CPML>0)) then
        call assemble_MPI_matrix_async_w_ord(NPROC,nglob_CPML,Qt_t, &
                        buffer_recv_matrix_PML,num_interfaces_PML, &
                        max_nibool_interfaces_PML, &
                        nibool_interfaces_PML,ibool_interfaces_PML, &
                        request_send_matrix_PML,request_recv_matrix_PML, &
                        my_neighbors_PML,myrank,t_wait)
        !do igll = 1, nglob_CPML
        !  Qt_t(:,:,igll) = Qt_t_ext(:,:,CPML_to_glob(igll))
        !enddo
      endif
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    endif
...
!        which at this point are stored in the array 'accel'
 ...
 ! multiplies with inverse of mass matrix (note: rmass has been inverted already)
  ...
  !                   call update_pml_aux_conv()
    !call compute_Qu_t(veloc)
    ! Qt^{n+1}=b*Qt^{n}+a*dQt^{n+1/2}/dt
    !call update_Qt_conv()
    ! a^{n+1} = ...Qt^{n+1}
    ! TL: timing
    t_clock = wtime()
    call include_adepml_accel_aux()
...
!!! It is better to move this into do iphase=1,2 loop
      if (ispec_is_elastic(ispec) .and. is_CPML(ispec)) then
        ...
        do igll = 1,NGLLSQUARE
          ...
          veloc(:,iglob) = 0._CUSTOM_REAL
          displ(:,iglob) = 0._CUSTOM_REAL
          ! if use ADE-PML then boundary values for auxiliary variables are
          ! automatically set
...
!   updates the velocity term which requires a(t+delta)
  ...
  if (USE_LDDRK) then
    ...
    ! v^{n+1}=v^{n+1/2}+1/2\delta t*a^{n+1}
  ...
  if (PML_CONDITIONS .and. USE_ADE_PML.and.(nglob_CPML>0)) then
    ! TL: timing
    t_clock = wtime()
    ! dQu^{n+1}/dt=...dv^{n+1}/dx
    !call compute_Qu_t(veloc)
    ! Qu^{n+3/2}=b*Qu^{n+1/2}+a*dQu^{n+1}/dt
    call update_Qt_conv()
    call update_Qu_conv()
    t_update_pml = t_update_pml + (wtime() - t_clock)
  endif
