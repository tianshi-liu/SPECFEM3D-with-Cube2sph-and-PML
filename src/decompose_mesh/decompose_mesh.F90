...
!! add by TL: implement regular partitioning
  logical, parameter :: DECOMPOSE_DEFAULT = .false.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! mesh arrays
  ...
  !! TL: add for load balancing on edges
  integer, dimension(:), allocatable  :: edges_load
...
!! DK DK I have no idea how this formula works; it was designed by Nicolas Le Goff
  ...
  subroutine scotch_partitioning()
    ...
    integer :: ier
    ! TL: add to loop over elements
    integer :: ispec,jspec,k_start,k_end,k
    ...
    allocate(adjncy(1:sup_neighbor*int(nspec,kind=8)),stat=ier)
    ...
    allocate(nodes_elmnts(1:nsize*int(nnodes,kind=8)),stat=ier)
...
!! DK DK Oct 2012: added this safety test
    ...
    ! TL: add for load balancing on edges
    allocate(edges_load(1:nb_edges),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 113')
    if (ier /= 0) stop 'Error allocating array edges_load'
...
!! DK DK Oct 2012: this should include CPML weights as well in the future
    ...
    !TL: adjust weights for CPML elements
    do ispec = 1, nspec
      if (is_CPML(ispec)) elmnts_load(ispec) = elmnts_load(ispec) * 5.5
    enddo
    !do ispec = 1, nspec
    !  k_start = xadj(ispec)+1
    !  k_end = xadj(ispec+1)
    !  do k = k_start,k_end
    !    jspec = adjncy(k)+1
    !    if (is_CPML(ispec) .and. is_CPML(jspec)) then
    !      edges_load(k) = 4
    !    else
    !      edges_load(k) = 1
    !    endif
...
! implement mesh repartitioning of poroelastic-elastic interface
! (the risk being to break the nice load balancing created by the domain decomposer for high-performance computing)
    !!! added by TL: to implement regular partitioning
    if (DECOMPOSE_DEFAULT) then
      call decompose_default_repartitioning(nspec, nnodes, elmnts, &
