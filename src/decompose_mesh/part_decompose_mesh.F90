       ...
       do k = 0, nnodes_elmnts(j)-1
          do l = k+1, nnodes_elmnts(j)-1
             ...
             if (connectivity >= ncommonnodes) then
                ...
                !! TL: modify to allow for very large mesh, taking advantage of
                !! nodes_elmnts(k+j*nsize) = elem_base
                !! nodes_elmnts(l+j*nsize) = elem_target
                do m = 0, xadj(elem_base)
                   if (.not. is_neighbor) then
                      if (adjncy(elem_base*sup_neighbor+m) == elem_target) then
                         is_neighbor = .true.
                      endif
                   endif
                enddo
                if (.not. is_neighbor) then
                   adjncy(elem_base*sup_neighbor &
                   ...
                   adjncy(elem_target*sup_neighbor &
                          + xadj(elem_target)) = elem_base

                   xadj(elem_target) = xadj(elem_target) + 1
                   if (xadj(elem_target) > sup_neighbor) &
                    stop 'ERROR: too many neighbors per element, error in the mesh or in the code.'
                endif
             endif
          enddo
...
! stores element indices for elements from above search at each interface
    ...
    do i = 1, count_def_mat
      ...
      !
       write(IIN_database) mat_prop(1,i), mat_prop(2,i), mat_prop(3,i), &
                           ...
                           mat_prop(13,i), mat_prop(14,i), mat_prop(15,i), mat_prop(16,i),mat_prop(17,i)
    enddo
  ...
  subroutine write_cpml_database(IIN_database, iproc, nspec, nspec_cpml, CPML_to_spec, &
    ...
    if (nspec_cpml > 0) then
       ...
       !!TL: for debug
       print *, 'rank ', iproc, ' has ', nspec_cpml_local, ' PML elements.'
  ...
  subroutine decompose_default_repartitioning(nspec, nnodes, elmnts, &
                       nodes_coords, nparts, part, NGNOD)
    implicit none
    integer, intent(in) :: nspec, nnodes, nparts, NGNOD
    ...
    integer :: ier, NPROC_X, NPROC_Y, ix, iy, ispec, ipart
    double precision :: xmin, xmax, ymin, ymax, xc, yc
    integer, dimension(0:nparts-1) :: part_count
    filename = "partition_default"
    open(unit=98, file=filename(1:len_trim(filename)), status='old', &
    ...
    print *, 'revert to default partitioning'
    part_count(:) = 0
    do ispec = 1, nspec
      xc = nodes_coords(1, elmnts(NGNOD, ispec)+1)
      yc = nodes_coords(2, elmnts(NGNOD, ispec)+1)
      if (xc < xmin) then
        ix = 0
      else if (xc > xmax) then
        ix = NPROC_X - 1
      else
        ix = floor((xc - xmin) / (xmax - xmin) * (1.0 * NPROC_X))
      endif
      if (yc < ymin) then
        iy = 0
      else if (yc > ymax) then
        iy = NPROC_Y
      else
        iy = floor((yc - ymin) / (ymax - ymin) * (1.0 * NPROC_Y))
      endif
      part(ispec) = iy * NPROC_X + ix
      if ((part(ispec).lt.0) .or. (part(ispec).gt.(nparts-1))) then
        print *, 'wrong rank', part(ispec)
      endif
      part_count(part(ispec)) = part_count(part(ispec))+1
    enddo
    do ipart = 0, nparts-1
      print *, 'rank ', ipart, ': ', part_count(ipart), ' elements'
    enddo
  end subroutine decompose_default_repartitioning
