...
!=====================================================================
  ...
  if (SAVE_MESH_AS_CUBIT) then
    ...
    if (NPROC_XI == 1 .and. NPROC_ETA == 1) then
      ...
      !! TL: modify to output CPML flags
      ...
      call save_output_mesh_files_as_cubit(nspec,nglob, ibool,nodes_coords, ispec_material_id, &
           nspec2D_xmin,nspec2D_xmax,nspec2D_ymin,nspec2D_ymax,NSPEC2D_BOTTOM,NSPEC2D_TOP, &
           NSPEC2DMAX_XMIN_XMAX,NSPEC2DMAX_YMIN_YMAX,NMATERIALS,material_properties, &
           ibelm_xmin,ibelm_xmax,ibelm_ymin,ibelm_ymax,ibelm_bottom,ibelm_top,nspec_CPML,CPML_to_spec,CPML_regions)
...
!---------------------------------------------------------------------------------------------------------------
  ...
  ! CPML
  integer :: nspec_CPML
  integer :: CPML_to_spec(nspec_CPML),CPML_regions(nspec_CPML)
  ...
  !! TL: add to output CPML flags
  if (nspec_CPML > 0) then
    open(IIN_database,file='MESH/absorbing_cpml_file')
    write(IIN_database,*)nspec_CPML
    do i =1,nspec_CPML
      write(IIN_database,'(2(i10,1x))')CPML_to_spec(i),CPML_regions(i)
    enddo
    close(IIN_database)
  endif
