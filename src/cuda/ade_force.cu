                    ...
                    const realw* Qu)
{
  ...
  const int idx1 = idx * NDIM * NDIM;
  const realw* rt = rtrans + idx1, *rtinv = rtrans_inv + idx1;
  #define Rmat(a,b)  rt[(b-1) * NDIM + a-1]
  ...
  k1l = 1.0f/ pml_kappa[idx * NDIM + 1 - 1];
  k2l = 1.0f/ pml_kappa[idx * NDIM + 2 - 1];
  k3l = 1.0f /pml_kappa[idx * NDIM + 3 - 1];

  pmlxxl = Rmat(1,1)*Rmatinv(1,1)*k1l + Rmat(2,1)*Rmatinv(2,1)*k2l + Rmat(3,1)*Rmatinv(3,1)*k3l;
  pmlxyl = Rmat(1,1)*Rmatinv(1,2)*k1l + Rmat(2,1)*Rmatinv(2,2)*k2l + Rmat(3,1)*Rmatinv(3,2)*k3l;
  pmlxzl = Rmat(1,1)*Rmatinv(1,3)*k1l + Rmat(2,1)*Rmatinv(2,3)*k2l + Rmat(3,1)*Rmatinv(3,3)*k3l;
  pmlyxl = Rmat(1,2)*Rmatinv(1,1)*k1l + Rmat(2,2)*Rmatinv(2,1)*k2l + Rmat(3,2)*Rmatinv(3,1)*k3l;
  pmlyyl = Rmat(1,2)*Rmatinv(1,2)*k1l + Rmat(2,2)*Rmatinv(2,2)*k2l + Rmat(3,2)*Rmatinv(3,2)*k3l;
  pmlyzl = Rmat(1,2)*Rmatinv(1,3)*k1l + Rmat(2,2)*Rmatinv(2,3)*k2l + Rmat(3,2)*Rmatinv(3,3)*k3l;
  pmlzxl = Rmat(1,3)*Rmatinv(1,1)*k1l + Rmat(2,3)*Rmatinv(2,1)*k2l + Rmat(3,3)*Rmatinv(3,1)*k3l;
  pmlzyl = Rmat(1,3)*Rmatinv(1,2)*k1l + Rmat(2,3)*Rmatinv(2,2)*k2l + Rmat(3,3)*Rmatinv(3,2)*k3l;
  pmlzzl = Rmat(1,3)*Rmatinv(1,3)*k1l + Rmat(2,3)*Rmatinv(2,3)*k2l + Rmat(3,3)*Rmatinv(3,3)*k3l;
  ...
  e11 = pmlxxl*duxdxl+pmlxyl*duxdyl+pmlxzl*duxdzl+ Rmat(1,1)*Qmat(1,1)+Rmat(2,1)*Qmat(2,1)+Rmat(3,1)*Qmat(3,1);
  e22 = pmlyxl*duydxl+pmlyyl*duydyl+pmlyzl*duydzl+ Rmat(1,2)*Qmat(1,2)+Rmat(2,2)*Qmat(2,2)+Rmat(3,2)*Qmat(3,2);
  e33 = pmlzxl*duzdxl+pmlzyl*duzdyl+pmlzzl*duzdzl+ Rmat(1,3)*Qmat(1,3)+Rmat(2,3)*Qmat(2,3)+Rmat(3,3)*Qmat(3,3);
  e12 = pmlxxl*duydxl+pmlxyl*duydyl+pmlxzl*duydzl+ Rmat(1,1)*Qmat(1,2)+Rmat(2,1)*Qmat(2,2)+Rmat(3,1)*Qmat(3,2)+ 
        pmlyxl*duxdxl+pmlyyl*duxdyl+pmlyzl*duxdzl+ Rmat(1,2)*Qmat(1,1)+Rmat(2,2)*Qmat(2,1)+Rmat(3,2)*Qmat(3,1);
  e13 = pmlxxl*duzdxl+pmlxyl*duzdyl+pmlxzl*duzdzl+ Rmat(1,3)*Qmat(1,3)+Rmat(2,3)*Qmat(2,3)+Rmat(3,3)*Qmat(3,3)+ 
        pmlzxl*duxdxl+pmlzyl*duxdyl+pmlzzl*duxdzl+ Rmat(1,1)*Qmat(1,1)+Rmat(2,1)*Qmat(2,1)+Rmat(3,1)*Qmat(3,1);
  e23 = pmlyxl*duzdxl+pmlyyl*duzdyl+pmlyzl*duzdzl+ Rmat(1,2)*Qmat(1,3)+Rmat(2,2)*Qmat(2,3)+Rmat(3,2)*Qmat(3,3)+ 
        pmlzxl*duydxl+pmlzyl*duydyl+pmlzzl*duydzl+ Rmat(1,3)*Qmat(1,2)+Rmat(2,3)*Qmat(2,2)+Rmat(3,3)*Qmat(3,2);
                    ...
                    realw_const_p pml_d, realw_p ax,realw_p ay, realw_p az)
{
  ...
  const realw *rt = &r_trans[idx * NDIM *NDIM];
  const realw *rt_inv = &r_trans_inv[idx * NDIM *NDIM];
  #define Rmat(i,j) rt[(j-1) * NDIM + i-1]
  #define Rmatinv(i,j) rt_inv[(j-1) * NDIM + i-1]
  #define TMAT(a,b,c)  temp[((c-1) * NDIM + (b-1)) * 6 + a-1]
  ...
  realw k1l = pml_kappa[1-1 + idx * NDIM];
  realw k2l = pml_kappa[2-1 + idx * NDIM];
  realw k3l = pml_kappa[3-1 + idx * NDIM];
...
__device__ __forceinline__ void 
add_pml_physical_contribution(int ispec_pml, int tid, const int *phy_spec,
                              ...
                              realw_p Qt_t)
{
  ...
  for(int id = tid; id < 6*NGLL2; id += NGLL3_PADDED) {
    int iside = tid % 6;
    int igll = tid / 6;
                      ...
                      const realw* Qu,realw_p Qu_t,realw_p Qt_t)
{
  ...
  if(!is_CPML[working_element]) {
    ...
    realw newtemp_adepml[54];
    int ispec_pml = spec_to_CPML[working_element] - 1;

    // copy rtrans,rtransinv
    realw rt[9],rtinv[9];
    for(int i = 0; i < 9; i ++) {
      int idx = ispec_pml *NGLL3 + K* NGLL2 + J * NGLLX + I;
      rt[i] = rtrans[idx*NDIM*NDIM+i];
      rtinv[i] = rtrans_inv[idx*NDIM*NDIM+i];
    }
    ...
    compute_sigma_adepml(ispec_pml,I,J,K,ANISOTROPY,d_c11store,d_c12store,d_c13store,
                        ...
                        rt,rtinv,pml_kappa,&sigma_xx,&sigma_xy,&sigma_xz,
    ...
    compute_accel_adepml(ispec_pml,I,J,K,newtemp_adepml,
                          fac1,fac2,fac3,rtrans,rtrans_inv,
