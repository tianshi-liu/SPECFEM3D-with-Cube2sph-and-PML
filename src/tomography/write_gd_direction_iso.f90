program write_gd_direction_iso
  use tomography_par
  ...
  dirname="OUTPUT_SUM"
  write(filename,'(a,i6.6,a)') trim(dirname)//'/proc',myrank,'_alpha_kernel_smooth.bin'
  open(1001,file=trim(filename),status='old',form='unformatted',iostat=ier)
  read(1001) model_dbulk
  write(filename,'(a,i6.6,a)') trim(dirname)//'/proc',myrank,'_beta_kernel_smooth.bin'
  open(1001,file=trim(filename),status='old',form='unformatted',iostat=ier)
  read(1001) model_dbeta
  write(filename,'(a,i6.6,a)') trim(dirname)//'/proc',myrank,'_rho_kernel_smooth.bin'
  open(1001,file=trim(filename),status='old',form='unformatted',iostat=ier)
  read(1001) model_drho

  model_dbulk(:,:,:,:) = - model_dbulk(:,:,:,:)
  if (USE_RHO_SCALING) then
    model_dbeta(:,:,:,:) = - model_dbeta(:,:,:,:) - model_drho(:,:,:,:) &
                           * RHO_SCALING
    model_drho(:,:,:,:) = model_dbeta(:,:,:,:) * RHO_SCALING
  else
    model_dbeta(:,:,:,:) = - model_dbeta(:,:,:,:)
    model_drho(:,:,:,:) = - model_drho(:,:,:,:)
  endif
  call write_gradient_iso()

  call synchronize_all()
  call finalize_mpi()
end program write_gd_direction_iso
