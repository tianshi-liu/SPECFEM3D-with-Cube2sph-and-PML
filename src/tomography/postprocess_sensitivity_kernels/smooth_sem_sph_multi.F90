...
program smooth_sem
  ...
  use constants, only: USE_QUADRATURE_RULE_FOR_SMOOTHING, HUGEVAL
  ...
  integer :: i,j,k,iglob,ier,ispec2,ispec,ispec_irreg,inum, iker
  ...
  real(kind=CUSTOM_REAL) :: sigma_h, sigma_h2, sigma_h3, sigma_v, sigma_v2, sigma_v3, sigma_M3
  ...
  real(kind=CUSTOM_REAL) :: r_min_ref,r_max_ref
  real(kind=CUSTOM_REAL) :: x_min,x_max
  real(kind=CUSTOM_REAL) :: y_min,y_max
  real(kind=CUSTOM_REAL) :: z_min,z_max
  real(kind=CUSTOM_REAL) :: r_min,r_max
  real(kind=CUSTOM_REAL) :: dim_x,dim_y,dim_z,dim_r
  ...
  ! add for sph smooth
  r_min_ref = minval(sqrt(xstore * xstore + ystore * ystore + zstore * zstore))
  r_max_ref = maxval(sqrt(xstore * xstore + ystore * ystore + zstore * zstore))

  dim_x = abs(x_max_ref - x_min_ref)
  dim_y = abs(y_max_ref - y_min_ref)
  dim_z = abs(z_max_ref - z_min_ref)
  ! add for sph smooth
  dim_r = abs(r_max_ref - r_min_ref)
  ...
  do iproc = 0,NPROC-1
    ...
    if (.not. do_include_slice) then
      ...
      z_max = maxval(zstore)
      ! add for sph smooth
      r_min = minval(sqrt(xstore * xstore + ystore * ystore + zstore * zstore))
      r_max = maxval(sqrt(xstore * xstore + ystore * ystore + zstore * zstore))
      ...
      if ((x_min < x_max_ref + sigma_M3 .and. x_max > x_min_ref - sigma_M3) &
          .and. (y_min < y_max_ref + sigma_M3 .and. y_max > y_min_ref - sigma_M3) &
          .and. (z_min < z_max_ref + sigma_M3 .and. z_max > z_min_ref - sigma_M3) &
          .and. (r_min < r_max_ref + sigma_v3 .and. r_max > r_min_ref - sigma_v3)) then
        do_include_slice = .true.
      endif
  ...
  allocate(tk(NGLLX,NGLLY,NGLLZ,NSPEC_AB,nker),stat=ier)
  ...
  do inum = 1,num_slices
    ...
    allocate(dat(NGLLX,NGLLY,NGLLZ,NSPEC_N,nker),stat=ier)
      ...
      do ispec = 1, NSPEC_AB
        ...
        do ispec2 = 1, NSPEC_N
            ...
            do k=1,NGLLZ
              do j=1,NGLLY
                do i=1,NGLLX
                  ...
                  ! TL: modify to ispec2
                  if (ispec_irreg /= 0) jacobianl = jacobian(i,j,k,ispec2)
          ...
          DO_LOOP_IJK
            ...
            !TL: add for debug
            if (any(abs(exp_val)>1.01)) print *, 'invalid exp_val ', myrank
            if (any(exp_val /= exp_val)) print *, 'nan exp_val ', myrank
            if (.not. abs(bk(INDEX_IJK,ispec)) < HUGEVAL) then
              print *, 'invalid bk ', myrank
              print *, 'ispec=', ispec, ' ispec2=', ispec2
            endif
            !!!!!!!!!!!!!!!!!!!!!
            ...
            do iker = 1, nker
            tk(INDEX_IJK,ispec,iker) = tk(INDEX_IJK,ispec,iker) + sum(exp_val(:,:,:) * dat(:,:,:,ispec2,iker))
            enddo
  ...
  allocate(dat_smooth(NGLLX,NGLLY,NGLLZ,NSPEC_AB,nker),stat=ier)
  ...
  if (USE_GPU) then
    ...
    do ispec = 1, NSPEC_AB
      ...
      DO_LOOP_IJK
        ...
        if (.not. (abs(bk(INDEX_IJK,ispec)) < HUGEVAL)) then
          print *, 'reach inf:', bk(INDEX_IJK, ispec)
          print *, 'myrank=', myrank
        endif

        ! normalizes smoothed kernel values by integral value of Gaussian weighting
        do iker = 1, nker
        dat_smooth(INDEX_IJK,ispec,iker) = tk(INDEX_IJK,ispec,iker) / bk(INDEX_IJK,ispec)
        enddo
  ...
  do iker = 1, nker
  ! statistics
  min_new(iker) = minval(dat_smooth(:,:,:,:,iker))
  max_new(iker) = maxval(dat_smooth(:,:,:,:,iker))
...
!
  ...
  real(kind=CUSTOM_REAL), parameter :: R_EARTH = 6371000.0
  real(kind=CUSTOM_REAL) :: r0, r1, theta, ratio
  integer :: i,j,k
...
! only for flat Earth with z in vertical direction
  ...
  dist_h = (R_EARTH * theta) * (R_EARTH * theta)
