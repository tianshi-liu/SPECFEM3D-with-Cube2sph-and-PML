...
program smooth_sem_sph_pde
  ...
  integer, parameter :: PRINT_INFO_PER_STEP = 100
  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:),allocatable :: dat,rotate_r
  ...
  integer :: i,j,k,l,iglob,ier,ispec,ispec_p,iphase,ispec_irreg,iker
  ...
  allocate(dat(nker,NGLLX,NGLLY,NGLLZ,NSPEC_AB))
  allocate(dat_glob(nker,NGLOB_AB))
  allocate(ddat_glob(nker,NGLOB_AB))
  allocate(dx_elem(nker,NGLLX,NGLLY,NGLLZ))
  allocate(dy_elem(nker,NGLLX,NGLLY,NGLLZ))
  allocate(dz_elem(nker,NGLLX,NGLLY,NGLLZ))
  allocate(stemp1(nker,NGLLX,NGLLY,NGLLZ))
  allocate(stemp2(nker,NGLLX,NGLLY,NGLLZ))
  allocate(stemp3(nker,NGLLX,NGLLY,NGLLZ))
  allocate(snewtemp1(nker,NGLLX,NGLLY,NGLLZ))
  allocate(snewtemp2(nker,NGLLX,NGLLY,NGLLZ))
  allocate(snewtemp3(nker,NGLLX,NGLLY,NGLLZ))
  allocate(buffer_send_vector_ext_mesh_smooth(nker,&
              max_nibool_interfaces_ext_mesh,num_interfaces_ext_mesh),stat=ier)
  allocate(buffer_recv_vector_ext_mesh_smooth(nker,&
              max_nibool_interfaces_ext_mesh,num_interfaces_ext_mesh),stat=ier)
  ...
  do iker = 1, nker
    ...
    local_data_file = trim(prname) // trim(kernel_names(iker)) // '.bin'
    ...
    ! statistics
    min_old(iker) = minval(dat(iker,:,:,:,:))
    max_old(iker) = maxval(dat(iker,:,:,:,:))
  enddo
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ! project
  dat_glob(:,:) = 0.0
  do iker = 1, nker
    do ispec = 1, NSPEC_AB
      do k=1,NGLLZ;do j=1,NGLLY;do i=1,NGLLX
        iglob = ibool(i,j,k,ispec)
        dat_glob(iker,iglob) = dat_glob(iker,iglob) + dat(iker,i,j,k,ispec) &
                               * rvol_local(i,j,k,ispec)
      enddo;enddo;enddo
    enddo
  enddo
  call assemble_MPI_vector_async_send_smooth(NPROC,NGLOB_AB,nker,&
  ...
  call assemble_MPI_vector_async_w_ord_smooth(NPROC,NGLOB_AB,nker,&
  ...
  do iker = 1, nker
    dat_glob(iker,:) = dat_glob(iker,:) * rvol(:)
    min_val = minval(dat_glob(iker,:))
    max_val = maxval(dat_glob(iker,:))
    call min_all_cr(min_val, min_val_glob)
    call max_all_cr(max_val, max_val_glob)
    if (myrank == 0) then
      print *, '  '//trim(kernel_names(iker))
  ...
  do istep = 1, ntstep
    ...
    do iphase = 1,2
      ...
      do ispec_p = 1,num_elements
        ...
        call get_gradient_element(dat(:,:,:,:,ispec),nker,&
    ...
    enddo !iphase = 1,2
    do iker = 1, nker
      ddat_glob(iker,:) = ddat_glob(iker,:) * rvol(:)
    enddo
    !! update
    dat_glob(:,:) = dat_glob(:,:) + ddat_glob(:,:)
    !! info
    if (mod(istep, PRINT_INFO_PER_STEP) == 0 .or. istep < 5) then
  ...
  do iker = 1, nker
    ! statistics
    min_new(iker) = minval(dat(iker,:,:,:,:))
    max_new(iker) = maxval(dat(iker,:,:,:,:))
  ...
  do iker = 1, nker
    call min_all_cr(min_old(iker), min_old_all(iker))
    call min_all_cr(min_new(iker), min_new_all(iker))
    call max_all_cr(max_old(iker), max_old_all(iker))
    call max_all_cr(max_new(iker), max_new_all(iker))

    if (myrank == 0) then
      print *
      print *, kernel_names(iker)
      print *,'Minimum data value before smoothing = ', min_old_all(iker)
      print *,'Minimum data value after smoothing  = ', min_new_all(iker)
      print *
      print *,'Maximum data value before smoothing = ', max_old_all(iker)
      print *,'Maximum data value after smoothing  = ', max_new_all(iker)
      print *
      close(IMAIN)
    endif
  enddo
  ...
  subroutine assemble_MPI_vector_async_send_smooth(NPROC,NGLOB_AB,nker,&
  ...
  ! array to assemble
  real(kind=CUSTOM_REAL), dimension(nker,NGLOB_AB) :: array_val

  integer :: num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh

  real(kind=CUSTOM_REAL), &
    dimension(nker,max_nibool_interfaces_ext_mesh,num_interfaces_ext_mesh) :: &
  ...
  if (NPROC > 1) then
    ...
    ! send messages
    do iinterface = 1, num_interfaces_ext_mesh
      call isend_cr(buffer_send_vector_ext_mesh_smooth(1,1,iinterface), &
                    nker*nibool_interfaces_ext_mesh(iinterface), &
                    my_neighbors_ext_mesh(iinterface), &
                    itag, &
                    request_send_vector_ext_mesh(iinterface))
      call irecv_cr(buffer_recv_vector_ext_mesh_smooth(1,1,iinterface), &
                    nker*nibool_interfaces_ext_mesh(iinterface), &
                    my_neighbors_ext_mesh(iinterface), &
                    itag, &
  ...
  end subroutine assemble_MPI_vector_async_send_smooth


  subroutine assemble_MPI_vector_async_w_ord_smooth(NPROC,NGLOB_AB,nker,&
...
! wait for communications completion (send)
  ...
  end subroutine assemble_MPI_vector_async_w_ord_smooth


  subroutine get_gradient_element(s, nker, dx_elem, dy_elem, dz_elem, &
     xix,xiy,xiz,etax,etay,etaz,gammax,gammay,gammaz)
  use constants, only: CUSTOM_REAL, NGLLX, NGLLY, NGLLZ
  use specfem_par, only: hprime_xxT,hprime_yyT,hprime_zzT
  implicit none
  integer :: nker
  real(kind=CUSTOM_REAL), dimension(nker, NGLLX, NGLLY, NGLLZ), intent(in) :: s
  real(kind=CUSTOM_REAL), dimension(nker, NGLLX, NGLLY, NGLLZ), intent(out) ::&
          dx_elem, dy_elem, dz_elem
  integer :: i,j,k,l
  real(kind=CUSTOM_REAL) :: hp1,hp2,hp3
  real(kind=CUSTOM_REAL), dimension(nker) :: temp1l, temp2l, temp3l
