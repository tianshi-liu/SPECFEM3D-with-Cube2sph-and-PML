...
!          i'm not sure if this makes a difference.
  ...
  character(len=MAX_STRING_LEN) :: grad_names(MAX_KERNEL_NAMES), &
                                   ...
                                   model0_paths(MAX_KERNEL_PATHS), &
                                   model1_paths(MAX_KERNEL_PATHS), &
  ...
  real(kind=CUSTOM_REAL),dimension(:,:,:,:,:), allocatable :: &
          q_vector,r_vector,gradient1,gradient0,model1,model0,&
  ...
  read(arg(3),'(a)') model_names_comma_delimited
  read(arg(4),'(a)') out_kernel_names_comma_delimited
  read(arg(5),'(a)') is_perturbation_comma_delimited
  read(arg(6),'(a)') out_kernel_path
  read(arg(7),'(a)') fn_store ! file that contain paths of current gradient and
                              ! all the s-y pairs
  ...
  do istore = 1, nstore_provided
    ...
    read(IIN,'(a)') model0_paths(istore)
    read(IIN,'(a)') model1_paths(istore)
  enddo
...
! set up coordinates of the Gauss-Lobatto-Legendre points
  ...
  do istore=nstore_provided,1,-1
    ...
    call read_kernel_from_path(kernel1_paths(istore),kernel_names,&
                               nker,gradient1)
    call read_kernel_from_path(model0_paths(istore),model_names,&
                               nmod,model0)
    ...
    if (myrank == 0) then
      ...
      print *, model0_paths(istore)
      print *, model1_paths(istore)
    endif
...
! Jorge Nocedal and Stephen Wright, "Numerical Optimization", Springer (2006)
  ...
  if (myrank == 0) then
    open(unit=IOUT, file='lbfgs_values', form='formatted', &
  ...
  deallocate(q_vector,r_vector,gradient1,gradient0,model1,model0,&
             gradient_diff,model_diff)
  call finalize_mpi()
...
!********************************************************
  ...
  end program write_lbfgs_direction
