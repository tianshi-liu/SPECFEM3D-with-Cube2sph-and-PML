...
!! TL: change this part to accomodate ADE-PML
    if (.not. USE_ADE_PML) then
      ...
      if (CALC_ADEPML_DAMPING) &
      call adepml_set_local_dampingcoeff(myrank,xstore_dummy,ystore_dummy,zstore_dummy)
      if (SET_ADEPML_GLOB) &
      call adepml_set_index_glob(myrank,xstore_dummy,ystore_dummy,zstore_dummy) 
    endif
  endif
...
! saves binary mesh files for attenuation
  ...
  !!TL: add for anisotropy storage
  deallocate(kappavstore,muvstore,eta_anistore, Gc_nondimstore, Gs_nondimstore)
...
! array with anisotropy
  ...
  allocate(kappavstore(NGLLX,NGLLY,NGLLZ,NSPEC_ANISO),stat=ier)
  allocate(muvstore(NGLLX,NGLLY,NGLLZ,NSPEC_ANISO),stat=ier)
  allocate(eta_anistore(NGLLX,NGLLY,NGLLZ,NSPEC_ANISO),stat=ier)
  allocate(Gc_nondimstore(NGLLX,NGLLY,NGLLZ,NSPEC_ANISO),stat=ier)
  allocate(Gs_nondimstore(NGLLX,NGLLY,NGLLZ,NSPEC_ANISO),stat=ier)
