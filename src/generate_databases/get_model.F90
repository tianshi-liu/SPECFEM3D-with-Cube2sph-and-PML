...
! *********************************************************************************
  ...
  if (ANISOTROPY .and. (IMODEL /= IMODEL_DEFAULT) .and. (IMODEL /= IMODEL_TOMO)) then
...
! reads in material parameters from external binary files
  ...
  !! TL: add this for binary anisotropic model
  ...
  case (IMODEL_GLL)
    !! TL: add this for binary anisotropic model
    if (ANISOTROPY) then
      call model_gll_aniso(myrank,nspec,LOCAL_PATH)
    else
      ! note:
